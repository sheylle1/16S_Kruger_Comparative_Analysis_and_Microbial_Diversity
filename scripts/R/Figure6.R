...
ggsave("outputs/figures/Genus.png", width = 20, height = 10, dpi = 600)
...
ggsave("outputs/figures/Fig6_1.png", width = 15, height = 6, dpi = 600)
...
nr
ggsave("outputs/figures/Fig6_2.png", width = 20, height = 6, dpi = 600)
...
r
ggsave("outputs/figures/Fig6_3.png", width = 10, height = 6, dpi = 600)
...
final_combined_plot <- (c_centered / nr_centered / r_centered / legend_spacer / wrap_elements(shared_legend)) +
  plot_layout(heights = c(0.9, 0.9, 0.9, 0.08, 0.5)) +  # increase last number for more space for legend
  plot_annotation(
...
for(ext in c("png","pdf","tiff")){
  ggsave(
    filename = paste0("outputs/Final/Figure6.", ext),
...
# Extract and save legend separately with 5 columns
library(ggplot2)
library(cowplot)
library(patchwork)

# Create a dummy plot with the legend (5 columns)
legend_plot_5col <- ggplot(long_data, aes(x = Sample, y = Abundance, fill = genus)) +
  geom_col(color = NA) +
  scale_fill_manual(
    values = genus_colors_v,
    name = "Bacterial Genus"
  ) +
...
separate_legend_5col <- cowplot::get_legend(legend_plot_5col)

# Calculate dynamic height and width based on number of genera
num_genera <- length(unique(long_data$genus))
rows_needed <- ceiling(num_genera / 5)  # 5 columns

# Save legend as a separate file
ggsave("outputs/figures/legend6.png", 
       plot = separate_legend_5col, 
       width = 14,  # Wider for 5 columns
       height = rows_needed * 0.5 + 1,  # Dynamic height based on rows
       dpi = 600,
       bg = "white")

# Also save as PDF for publication quality
ggsave("outputs/figures/legend6.pdf", 
       plot = separate_legend_5col, 
       width = 14, 
       height = rows_needed * 0.5 + 1,
       bg = "white")


# Optional: Create a standalone legend function for more control
create_multi_col_legend <- function(color_vector, legend_title = "Bacterial Genus", 
                                     ncol = 5, text_size = 11, title_size = 16) {
  
  # Create a data frame for the legend
  legend_df <- data.frame(
    Genus = factor(names(color_vector), levels = names(color_vector)),
    x = 1,
    y = 1
  )
  
  # Create plot with only legend
  p <- ggplot(legend_df, aes(x = x, y = y, fill = Genus)) +
    ...
    guides(fill = guide_legend(
      ncol = ncol,
      override.aes = list(size = 3),
      label.theme = element_text(size = text_size, face = "italic")
    )) +
    theme_void() +
    theme(
      legend.title = element_text(size = title_size, face = "bold"),
      legend.text = element_text(size = text_size, face = "italic"),
      legend.key.size = unit(0.6, "cm"),
      legend.spacing.y = unit(0.1, "cm"),
      legend.spacing.x = unit(0.2, "cm"),
      legend.background = element_rect(fill = "white", color = "black", linewidth = 0.5),
