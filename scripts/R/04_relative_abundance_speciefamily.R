...
ggplot(c_data, aes(x = Animal, y = Abundance, fill = Family)) +
...
# ==== Final plot ====
ggplot(nr_data, aes(x = Animal, y = Abundance, fill = Family)) +
  geom_col(color = "black", size = 0.2) +
  facet_wrap(~ Species, scales = "free_x", nrow = 1) +
  scale_fill_manual(values = family_colors) +
  theme_bw(base_size = 16) +  
...
ggsave("output/figures/microbiome_NR_Family.png", width = 15, height = 6, dpi = 600)
...
ggplot(r_data, aes(x = Animal, y = Abundance, fill = Family)) +
  geom_col(color = "black", size = 0.2) +
  facet_wrap(~ Species, scales = "free_x", nrow = 1) +
  scale_fill_manual(values = family_colors) +
  theme_bw(base_size = 16) +  
...
ggsave("output/figures/microbiome_R_Family.png", width = 15, height = 6, dpi = 600)