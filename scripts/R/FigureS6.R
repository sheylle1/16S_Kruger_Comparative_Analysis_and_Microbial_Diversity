...
data <- read.csv("D:/01_Work/02_ActiveProjects/Project_16S_Kruger/Manuscript_analysis/data/raw/level-6.csv", check.names = FALSE)

# Remove outliers
#outliers <- c("K058683-R", "K058706-R", "K058789-R", "K058689-R", "K058814-R", "K058785-R", "K058700-R", "K058796-R", "K058824-R", "K058810-R", "K058694-R")
...
custom_plot1 <- ggplot() +
  ...
  labs(x = "Mean Decrease Accuracy", y = "Taxonomic Genera") +
...
legend_plot <- ggplot(importance_long_char,
  ...
  theme(
    ...
    legend.key.size = unit(1.5, "lines")                    # Size of color boxes
  )
...
for (ext in c("png", "pdf", "tiff")) {
  ggsave(
    filename = paste0("outputs/figures/FigureS6.", ext),
...
for (ext in c("png")) {
  ggsave(
    filename = paste0("outputs/Final/FigureS6.", ext),
    plot = final_combined_plot,
    width = 16,
    height = 10,
    dpi = 600
